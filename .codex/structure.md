...
Command Palette
  ...
  -> writes kawaii-vscode-colors-ui.js into VS Code workbench folder
...
Disabling removes the HTML reference to `kawaii-vscode-colors-ui.js`. The current code does not delete the generated script file.
...
Command Palette
  ...
  -> extension host writes a [Kawaii VS Code Color] or [Kawaii VS Code Color Light] block to user settings
...
- The configuration listener watches `workbench.colorTheme`; when the user switches between `Kawaii VS Code Color` and `Kawaii VS Code Color Light` while Neon is already enabled, it regenerates the patched renderer script.
...
- It writes `kawaii-vscode-colors-ui.js` next to the workbench HTML.
...
- Keep the script URL versioned so workbench reload can pick up regenerated `kawaii-vscode-colors-ui.js` content.
