...
VS Code loads patched workbench HTML
  ...
  -> script adds .kawaii-vscode-colors-ui to document.documentElement when active
  ...
  -> token glow CSS is emitted as scoped additive rules
...
package.json contributes.configuration
  -> user settings.json
  -> enableNeon() reads workspace configuration when the Neon action runs
...
- `enableNeon()` reads `kawaii_synthwave.brightness` and `kawaii_synthwave.disableGlow` at execution time.
